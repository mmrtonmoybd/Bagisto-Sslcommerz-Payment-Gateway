    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != '' && $merchant_trans_amount != 0) {
            ...
            if ($this->SSLCOMMERZ_hash_verify($post_data, $this->getStorePassword())) {
                ...
                $requested_url = ($SSLCOMMERZ_BASE_URL.$this->config['apiUrl']['order_validate'].'?val_id='.$val_id.'&store_id='.$store_id.'&store_passwd='.$store_passwd.'&v=1&format=json');
...
//                    }
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $this->setApiUrl($SSLCOMMERZ_BASE_URL.$this->config['apiUrl']['make_payment']);
    ...
    {
        $this->successUrl = route($this->config['success_url']);
    }
    ...
    {
        $this->failedUrl = route($this->config['failed_url']);
    }
    ...
    {
        $this->cancelUrl = route($this->config['cancel_url']);
    }
